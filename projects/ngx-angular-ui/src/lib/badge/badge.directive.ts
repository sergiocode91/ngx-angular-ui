...
export class HtmlBadgeDirective implements OnInit, OnChanges {
  ...
  private applyClasses() {
    ...
    const classes = `${variantClass} ${sizeClass} ${colorClass} ${roundedClass} ${this.class} inline-flex items-center justify-center font-medium text-xs transition-colors`;
  ...
  private getVariantClass(variant: string): string {
    switch (variant) {
      ...
      case 'outline':
        return `shadow-sm ${this.color ? '' : 'text-zinc-900 border border-zinc-200 bg-white hover:bg-zinc-100 dark:text-neutral-50 dark:border-zinc-800 dark:bg-transparent dark:hover:bg-zinc-800'}`;
      default:
