...
export class HtmlInputDirective implements OnInit, OnChanges {
  ...
  private applyClasses() {
    const baseClasses =
      'w-full h-9 flex font-normal text-sm placeholder:text-zinc-500 px-3 py-1 shadow-sm rounded-md bg-transparent border border-input focus:outline-2 focus:outline-neutral-500 disabled:cursor-not-allowed disabled:opacity-50 dark:text-white dark:placeholder:text-white/70 dark:border-zinc-800 dark:focus:outline-zinc-800';
